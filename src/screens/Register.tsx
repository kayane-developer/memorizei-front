import { VStack, Text, Box, FormControl, Input, useToast } from 'native-base';
...
export default function Register({ navigation }: any) {
  ...
  const { handleSubmit, control, formState: { errors } } = useForm({
    resolver: yupResolver(schema), defaultValues: { usuario: '', senha: '' }
  })
  ...
  return (
    <VStack flex={1} alignItems={'center'}>
      ...
      <Box mr={'10%'} mt={'37%'}>
        <Text fontSize={23} fontWeight={'bold'}>Vamos criar seu usuário para começarmos a usar o</Text>
        <Text
          fontSize={23}
          fontWeight={'bold'}
          position={'absolute'}
      ...
      <ButtonComponent onPress={handleSubmit(result)}>Registrar</ButtonComponent>
      <Text
        color={'orange.500'}
        fontWeight={'bold'}
        fontSize={'md'}
